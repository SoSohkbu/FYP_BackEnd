...
router.post("/save-report", async function (req, res) {
  try {
    ...
    if (!reportName || !analyzedData || analyzedData.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Missing report name or data." });
    }
    ...
    const reportDoc = {
      ...
      recordType: req.body.recordType || "batch",
      createdAt: new Date(),
    };
...
router.get("/", async function (req, res) {
  try {
    ...
    const formattedBatch = batchHistory.map((item) => {
      ...
      const actualType =
        item.recordType ||
        (item.sourceName && /^https?:\/\//i.test(item.sourceName)
          ? "url-analyzer"
          : "batch");
      const typeLabel =
        actualType === "url-analyzer" ? "URL Analysis" : "Batch Report";
      ...
      return {
        ...
        originalText: `[${typeLabel}] ${item.reportName} - Analysed ${total} sentences from ${item.sourceName}.`,
        scores: {
    ...
    const combinedHistory = [...formattedSingle, ...formattedBatch].sort(
